...
from data.util import draw_3DBBox, vis_colors, vis_styles, compute_3d_bbox, corners8_to_rect4, read_image
from data.util import get_K, get_P, get_Rz, inverse

MULTIVIEWC_BBOX_LABEL_NAMES = ['Cow']

class MultiviewC_dataset(object):
    def __init__(self, root = r'.\MultiviewC', # PATH of MultiviewC dataset
        ...
        """
        assert bbox_mode in ['2D', '3D'], 'bbox mode must be `2D` or `3D`'
        assert train_mode in ['train', 'test'], 'train mode must be `train` or `test`.'
        ...
        if not read_img_ann:
            self.images_list, self.annotations_list = self.generate(bbox_mode)
        else:
...
class DetectedObject(object):
    def __init__(self, img, detection_class, bbox_2d, K_matrix, label=None) -> None:
        ...
        self.proj_matrix = np.array(K_matrix)
...
if __name__ == '__main__':
    MC = MultiviewC_dataset(bbox_mode='2D', train_mode='train', save_img_ann=False, read_img_ann=True)
    img, label = MC[10]
    img = inverse(img)
    # print(label)
    # print(img.shape)
